...
class Linear:
    ...
    def __init__(self, num_input, num_output):
        ...
        print("linear层init完成")
    def init_param(self,loc=0,scale=0.5):
        """loc：均值，scale：方差.
    ...
    def forward(self,input):
        ...
        output:shape=(batch,1,output_len)
        """
...
class SoftmaxAndCrossEntropy():
    ...
    def forward(self, input):
        """input:shape=(batch,1,input_len)
            ...
            inputlen 应当对应几种类别
        """
        assert len(input.shape)==3 and input.shape[1]==1 , f"inputshap{len(input.shape)},inpshape1{input.shape[1]}"
